...
def plot_3D(S0, K, T, sigma, r, Exp1, Exp2, M, N, scheme, restrict=True, savefig=False):
    ...
    t, S = np.meshgrid(np.linspace(1, 0, N+1), S_list)


    ax.plot_surface(t, S, V, cmap='binary', linewidth=0, antialiased=True)
    ax.view_init(20, 100)
    ax.set_xlabel('t')
    ax.set_ylabel('S')
...
def compute_and_plot_delta(S0=100, K=110, T=1, sigma=0.3, r=0.04, Exp1=-5, Exp2=7, M=2000, N=1000, scheme='CN',restrict=True, S_min=10, S_max=200, savefig=False, error=False):
...
compute_and_plot_delta(M=2000, N=1000, scheme='FTCS', restrict=True, error = True)

compute_and_plot_delta(M=2000, N=1000, scheme='FTCS', restrict=True)
plt.savefig(f'delta.pdf', dpi=200)
plt.show()
